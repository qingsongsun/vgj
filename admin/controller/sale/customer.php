...
class ControllerSaleCustomer extends Controller {
	...
	protected function getForm() {
		...
		$data['button_credit_adjust'] = $this->language->get('button_credit_adjust'); // Wenliang added
		...
		$data['user_group'] = $customer_group_info['name'];   // Wenliang added
...
/* Wenliang removed, for Chinese naming style
		if ((utf8_strlen($this->request->post['lastname']) < 1) || (utf8_strlen(trim($this->request->post['lastname'])) > 32)) {
			$this->error['lastname'] = $this->language->get('error_lastname');
		}
*/

/* Wenliang removed, not allowing update email
...
//		if ((utf8_strlen($this->request->post['telephone']) < 3) || (utf8_strlen($this->request->post['telephone']) > 32)) {
		...
		$customer_group_info = $this->model_sale_customer->getCustomerGroup($this->request->post['customer_group_id']);

		if ($customer_group_info['name'] == GROUP_NAME_VIP_USER) {
	        if (!preg_match("/^1[34578]\d{9}$/", trim($this->request->post['telephone']))) {
			    $this->error['telephone'] = $this->language->get('error_telephone');
		    } else {
		    	$customer = $this->model_sale_customer->getCustomerByPhone(trim($this->request->post['telephone']));
		    	// allowing own phone number change
		    	if ($customer && $this->request->get['customer_id'] != $customer['customer_id']) {
				    $this->error['telephone'] = $this->language->get('error_telephone_associated');
			    }
			}
		}
...
*/
	...
	public function transaction() {
		...
		// Wenliang added, for showing the authorized credit
		$query = $this->model_sale_customer->getVipInfoByCustomerId($this->request->get['customer_id']);
		$data['total_credit'] = sprintf($this->language->get('text_total_credit'), $this->currency->format($query->row['credit_amount']));
    ...
    // Wenliang added, for adjusting authorized credit and transaction together
	public function credit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/customer')) {
			$this->model_sale_customer->addTransaction($this->request->get['customer_id'], $this->request->post['description'], $this->request->post['amount']);
		    $query = $this->model_sale_customer->getVipInfoByCustomerId($this->request->get['customer_id']);
		    $new_credit = $query->row['credit_amount'] + $this->request->post['amount'];

    		$this->model_sale_customer->setVipInfoByCustomerId($this->request->get['customer_id'], $new_credit);
 		    $data['total_credit'] = sprintf($this->language->get('text_total_credit'), $this->currency->format($new_credit));
			$data['success'] = $this->language->get('text_success');
		} else {
			$data['success'] = '';
		}
