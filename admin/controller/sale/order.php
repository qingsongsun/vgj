...
class ControllerSaleOrder extends Controller {
	...
	public function edit() {
		...
		if ($this->validate()) {
			...
			$api_info = $this->model_user_api->getApi($this->config->get('config_api_id'));
// 			var_dump($api_info);
// var_dump($this->config->get('config_api_id'));exit();
	...
	protected function getList() {
		...
		// huwen added
		$data['sort_subtotal'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.subtotal' . $url, 'SSL');
		//
	...
	// huwen added
		...
		if (!empty($order_info)) {
			...
			*/
			...
			$products = $products = $this->model_sale_order->getOrderProductsImage($this->request->get['order_id']);

			foreach ($products as $product) {
				$options = $this->model_sale_order->getOrderOptionsImage($this->request->get['order_id'], $product['order_product_id']);
				foreach($options as $key=>$value){
					$options[$key]['image'] = $options[$key]['image']&&$options[$key]['image']!="no_image.png" ? $this->model_tool_image->resize($options[$key]['image'],80,80):null;
				}
				$data['order_products'][] = array(
					...
					'image'      => $product['image'] ? $this->model_tool_image->resize($product['image'],80,80):$this->model_tool_image->resize('no_image.png',80,80)
				);
			}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['entry_shipping_id'] = $this->language->get('entry_shipping_id');
			$data['warning_shipping_id'] = $this->language->get('warning_shipping_id');
			...
			$data['firstname'] = $order_info['firstname'];
			$data['lastname'] = $order_info['lastname'];
			// huwen added for user checkout
			if ($order_info['user_id']) {
				$data['user']=$order_info['user_name'];
				$data['user_checkout']='是';
			}else{
				$data['user_checkout']='否';
			}
			...
			*/
			...
			$data["shipping_company_list"] = $shipping_company;
			$this->load->model('tool/image');
			$products = $this->model_sale_order->getOrderProductsImage($this->request->get['order_id']);
			foreach ($products as $product) {
				$option_data = array();
				/* ishank removed it
				...
				*/
				...
				foreach ($options as $option) {
					if ($option['barcode']==null) {
						$data['barcode']=$product['model'];
					}else{
						$data['barcode']=$option['barcode'];
					}
				}
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						$option_data[] = array(
							...
							'value' => $option['value'],
							'type'  => $option['type'],
							'image' => ($option['image']&&$option['image']!='no_image.png')? $this->model_tool_image->resize($option['image'], 80, 80) : null
						);
					} else {
				...
				$order_repo_data = array(
					'order_product_id' => $product['order_product_id'],
					'product_id'=>$product['product_id'],
					'quantity'=> $product['quantity']
					);
				$data['repositories'][$product['order_product_id']] = $this->model_catalog_repository->getOrderProductRepository($order_repo_data);
				$data['available_repos'][$product['order_product_id']] = $this->model_catalog_repository->getRepository_quantitybyId($order_repo_data);
				$this->log->write($data['available_repos'][$product['order_product_id']]);
			...
			// Luke added begin
			// For order repository setting
			$data['text_no_repo_selected'] = $this->language->get('text_no_repo_selected');
			$data['column_repository'] = $this->language->get('column_repository');
			$data['column_is_deliver'] = $this->language->get('column_is_deliver');
			$data['entry_order_repository'] = $this->language->get('entry_order_repository');
			$data['button_deliver'] = $this->language->get('button_deliver');
			// $data['repositories'] = array(
			// 	11751=>array(3=>array('name'=>'Minhang', 'quantity' => 30)),
			// 	11752=>array(3=>array('name'=>'Minhang', 'quantity' => 30))
			// );
			// $data['available_repos'] = array(
			// 	11751=>array(
			// 		array('id'=>1,'name'=>'Xinzhuang'),
			// 		array('id'=>2,'name'=>'Xuhui'),
			// 		array('id'=>3,'name'=>'Minhang')
			// 	),
			// 	11752=>array(
			// 		array('id'=>1,'name'=>'Xinzhuang'),
			// 		array('id'=>2,'name'=>'Xuhui'),
		...
		if ($this->validate()) {
			...
			if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
				...
				if ($this->request->post) {
					// huwen added
					$this->request->post['user']=$this->user->getUserName();
					curl_setopt($curl, CURLOPT_POST, true);
...
$this->log->write($json);
	...
	//Zark added begin
	public function order_repo(){
		$this->load->model('catalog/repository');
		$json = file_get_contents("php://input");
		$request = json_decode($json,true);
		$response = array();
		foreach ($request as $order_item) {
			if(isset($order_item['repository_id']) && $order_item['repository_id']){
				$data = array(
					'order_product_id' => $order_item['order_product_id'],
					'repository_id' => $order_item['repository_id'],
					'quantity' => $order_item['quantity'],
					'user_id' => $this->user->getId()
					);
				$this->model_catalog_repository->addOrderProductRepository($data);
				$this->model_catalog_repository->delRepository_pd_order($data);
				$response[$order_item['order_product_id']] = true;
			} else {
				$response[$order_item['order_product_id']] = false;
			}
		}
		$json = json_encode($response);
