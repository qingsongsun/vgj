...
// Wenliang mod, we don't modify email, and implicitly not modifying telephone for non-VIP user due to invisble phone field in tpl

		$customer_group_info = $this->getCustomerGroup($data['customer_group_id']);


		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
...
//		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', approved = '" . (int)$data['approved'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
		...
		// additionally, set payment password for VIP user
		if ($customer_group_info['name'] == GROUP_NAME_VIP_USER) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer_payment SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', pay_password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
	...
	public function deleteCustomer($customer_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_coupon WHERE customer_id= '" .(int)$customer_id ."'");
	}
	...
	// Wenliang added for getting customer group info by ID
	...
	// Wenliang added, set everything to vip card database
	public function setVipInfoByCustomerId($customer_id, $credit_amount) {
		$this->db->query("UPDATE " . DB_PREFIX . "vip_card SET credit_amount = " . $credit_amount . " WHERE customer_id = '" . (int)$customer_id . "'");
	}
