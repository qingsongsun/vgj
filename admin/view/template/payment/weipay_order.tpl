...
<div class="alert alert-success" id="weipay_transaction_msg" style="display:none;"></div>
<table class="table table-striped table-bordered">
  <tr>
	<td><?php echo $text_order_ref; ?></td>
	<td><?php echo $weipay_order['weipay_transaction_id']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_order_total; ?></td>
	<td><?php echo $weipay_order['total_formatted']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_total_released; ?></td>
	<td id="weipay_total_released"><?php echo $weipay_order['total_released_formatted']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_refund_status; ?></td>
	<td id="refund_status">
	  <?php if ($weipay_order['refund_status'] == 1) { ?>
		  <span class="refund_text"><?php echo $text_yes; ?></span>
	  <?php } else { ?>
		  <span class="refund_text"><?php echo $text_no; ?></span>&nbsp;&nbsp;

		  <?php if ($weipay_order['total_released'] > 0) { ?>
  ...
  <tr>
	...
	<td>
	  <table class="table table-striped table-bordered" id="weipay_transactions">
		<thead>
		  <tr>
		...
		<tbody>
		  <?php foreach ($weipay_order['transactions'] as $transaction) { ?>
			  <tr>
...
//      if (confirm('<?php echo $text_confirm_refund ?>')) {
        $.ajax({
          ...
          url: 'index.php?route=payment/weipay/refund&token=<?php echo $token; ?>',
          beforeSend: function () {
//            $('#btn_refund').hide();
//            $('#refund_amount').hide();
//            $('#img_loading_refund').show();
//            $('#weipay_transaction_msg').hide();
          },
          success: function (data) {
            if (data.error == false) {
              ...
              $('#weipay_transactions').append(html);
              $('#weipay_total_released').text(data.data.total_released);
              ...
              if (data.msg != '') {
                $('#weipay_transaction_msg').empty().html('<i class="fa fa-check-circle"></i> ' + data.msg).fadeIn();
              }
            }
            if (data.error == true) {
              alert(data.msg);
