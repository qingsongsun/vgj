		...
		success: function(json) {
			...
			if (json['products']) {
				for (i = 0; i < json['products'].length; i++) {
					...
					html += "<?php $flag = 1 ?>";
					html += "<?php foreach ($order_product['option'] as $option) { ?>";
                    html += "<?php if ($option['type'] != 'file') { ?>";
                	html += "<?php if($option['image']){ ?>";
                	html += "			<img src=\"<?php echo $option['image'];?>\"/>";
                	html += "	<?php $flag=0 ?>";
                	html += "	<?php } ?>";
                    html += "<?php } ?>";
                    html += "<?php } ?>";
                    html += "<?php if ($flag){ ?>";
                  	html += "<img src=\"<?php echo $order_product['image']; ?>\" />";
                  	html += "<?php } ?>";
                    html += '</td>';
