...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
		...
		$data['button_view'] = $this->language->get('button_view');
                $data['button_pay'] = $this->language->get('text_pay');
        ...
        // Wenliang added, getting the cusotmer group info
...
// if the customer is non-VIP user, show total as the one actually paid
// else show the credit deducted for VIP user
		  if ($customer_group_info['name'] != GROUP_NAME_VIP_USER) {
			$data['orders'][] = array(
				...
				'href'       => $this->url->link('account/order/info', 'order_id=' . $result['order_id'], 'SSL'),
                                'payment_href' => $this->url->link('payment/weipay/payment', 'order_id=' . $result['order_id'], 'SSL'),
			);
		  } else {
			...
			$data['orders'][] = array(
				...
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'products'   => ($product_total + $voucher_total),
				'total'      => $this->currency->format($credit, $result['currency_code'], $result['currency_value']),
				'href'       => $this->url->link('account/order/info', 'order_id=' . $result['order_id'], 'SSL'),
                                'payment_href' => $this->url->link('payment/weipay/payment', 'order_id=' . $result['order_id'], 'SSL'),
			);
		  }
		...
		$data['continue'] = $this->url->link('account/account', '', 'SSL');
                $order_status = $this->model_localisation_order_status->getOrderStatus('2');
                $data['wait_for_payment_status'] = $order_status['name'];
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_shipping_search'] = $this->language->get('text_shipping_search');
			$data['text_shipping_id']     = $this->language->get('text_shipping_id');
			...
			if ($order_info['shipping_name_value']) {
				$data['shipping_name_value'] = $order_info['shipping_name_value'];
			} else {
				$data['shipping_name_value'] = '';
			}
			...
			foreach ($products as $product) {
				...
				// Huwen added,for image in the order
