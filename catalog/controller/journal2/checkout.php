...
class ControllerJournal2Checkout extends Controller {
    ...
    // Wenliang added, for updating the shipping price if needed
    public function updateShippingPrice() {
        //    if (!isset($this->data['addresses'])) return;
		...
		switch ($zone_code) {
				// huwen added
			case 'SJTU_MH':
				$this->session->data['shipping_method']['cost'] = DELIVERY_SJTU_MH;
				break;
			case 'JI':
			case 'ZH':
			case 'SG':
				//          $this->data['shipping_methods']['flat']['quote']['flat']['cost'] = DELIVERY_JIANG_ZHE_HU;
				//          $this->session->data['shipping_methods']['flat']['quote']['flat']['cost'] = DELIVERY_JIANG_ZHE_HU;
				$this->session->data['shipping_method']['cost'] = DELIVERY_JIANG_ZHE_HU;
				break;
			default:
				//          $this->data['shipping_methods']['flat']['quote']['flat']['cost'] = DELIVERY_OTHER_AREA;
				//          $this->session->data['shipping_methods']['flat']['quote']['flat']['cost'] = DELIVERY_OTHER_AREA;
				$this->session->data['shipping_method']['cost'] = DELIVERY_OTHER_AREA;
				break;
		}
	}

    public function index() {
        // Wenliang added, not allowing login/registration @ checkout
		...
		if ($this->isLoggedIn()) {
            ...
            if (isset($this->session->data['shipping_method'])&&$this->session->data['shipping_method']['code']=='pickup.pickup') {
                ...
                $this->data['repositories_able']=$this->model_catalog_repository->getRepository_quantitybyId($filter_data);


                $this->data['shipping_address'] = $this->renderAddressForm_pickup('pickup');

            }else{
                $this->data['shipping_address'] = $this->renderAddressForm_pickup('shipping');
            }


            // $this->data['pickup_address'] = $this->renderAddressForm('pickup');
		...
		// huwen added
		isset($this->session->data['shipping_method']['code'])?$this->data['shipping_method'] = $this->session->data['shipping_method']['code']:$this->data['shipping_method'] = '';


		// Wenliang added, only switched user needs payment password verification

        if (isset($this->session->data['switched_user'])) {// not allowed for switched user
            $this->data['is_switched_user'] = $this->session->data['switched_user'];
        } else {
            $this->data['is_switched_user'] = '';
        }

        //        $this->load->model('account/customer_group');
        $customer_group_info = $this->model_account_customer_group->getCustomerGroup($this->customer->getGroupId());

        $this->data['user_group'] = $customer_group_info['name'];
		...
		// huwen add
		$comment = $this->model_journal2_checkout->getComment();

		$this->load->language('checkout/checkout');

		if (isset($this->session->data['shipping_method']['code']) && $this->session->data['shipping_method']['code'] == "pickup.pickup") {
			if ($comment == null) {
				$this->data['pickup_error_comment'] = $this->language->get('pickup_error_comment');
			}
		}
		//
        ...
        // Wenliang added, for payment password verification
        $this->data['text_payment_password']  = $this->language->get('text_payment_password');
        $this->data['text_forgotten']         = $this->language->get('text_forgotten');
        $this->data['entry_payment_password'] = $this->language->get('entry_payment_password');
    ...
    public function confirm() {
		...
		if (isset($this->session->data['shipping_method']['code']) && $this->session->data['shipping_method']['code'] == "pickup.pickup") {
            ...
            // $this->session->data['token_code']=md5(mt_rand());

			if ($this->request->post['comment'] == null) {
				// $this->validateComment($this->request->post['comment']);
				$errors['comment'] = $this->language->get('error_pickup_comment');
			}
		}

		if ($this->isLoggedIn()) {
			// Wenliang added, for payment password validation, only for switched user
			if (isset($this->session->data['switched_user']) && !$this->customer->hasPaymentPassword(Journal2Utils::getProperty($this->request->post, 'password_validation'))) {
				//  $errors = array_merge($errors, $this->validateAddressData($new_payment_address, 'payment_'));
				$errors['password'] = $this->language->get('error_payment_password');
			}

            // payment data, modified by Wenliang, since we don't provide separate payment address, so use shipping address instead
    ...
    public function shipping($return = false) {
		...
		// huwen added
		$this->data['pickup_help']        = $this->language->get('pickup_help');
		$this->data['flat_help']          = $this->language->get('flat_help');
		$this->data['pickup_description'] = $this->language->get('pickup_description');
		$this->data['flat_description']   = $this->language->get('flat_description');
        ...
        // Wenliang added, for modifying the shipping price
...
// huwen added
        ...
        if (strpos($user_agent, 'MicroMessenger') === false) {
            ...
            $this->data['isWeixin']=false;
            // $this->data['weipay_payment_method']=$this->data['payment_methods']['weipay'];

        } else {
            $this->data['isWeixin']=true;
            $this->data['weipay_payment_method']=$this->data['payment_methods']['weipay'];
        }
 //
    ...
    public function pickupAddress(){
        ...
        if (isset($this->request->get['pickup'])) {

            // $this->renderAddressForm_pickup('pickup');
            $this->response->setOutput($this->renderAddressForm_pickup('pickup'));

        }else{
             $this->response->setOutput($this->renderAddressForm('shipping'));
        }

    }

    private function renderAddressForm_pickup($type, $name = true) {
        ...
        if ($type=="pickup") {
            return $this->renderView($this->config->get('config_template').'/template/journal2/checkout/pickup_address_form.tpl');
        }else{
            return $this->renderView($this->config->get('config_template').'/template/journal2/checkout/address_form.tpl');
        }
    ...
    private function validateAddressData($data, $key, $name = true) {
        ...
        if ($name) {
            ...
            // lastname
            /* Wenliang removed, for Chinese naming conversion
		...
		if ((utf8_strlen(trim($data[$key.'address_1'])) < 1) || (utf8_strlen(trim($data[$key.'address_1'])) > 128)) {
			$errors[$key.'address_1'] = $this->language->get('error_address_1');
		}

		if ((utf8_strlen($data[$key.'city']) < 1) || (utf8_strlen($data[$key.'city']) > 32)) {
    ...
    // Wenliang commented: This is never called for this release since it is not allowed to register/login @checkout
    public function login() {
        ...
        // Wenliang added for simplied registration
