<?php
/*
 本支付宝担保接口支付插件由 Yang Zhao Feng (杨兆锋) 开发，并授权仅在http://www.opencart.com, http://www.opencart.com.hk, http://www.chinaopencart.com 上销售，任何其它销售的地方，均为侵权违法行为。 如有任何问题请联系 1487063622@qq.com
*/
class ControllerPaymentAlipayDirect extends Controller {
	...
	$timeStamp = time();
        $out_trade_no = $timeStamp . "alipay". $this->session->data['order_id'];
        //商户网站订单系统中唯一订单号，必填
	...
	public function callback() {
		...
		if($verify_result) {//验证成功
			...
			$order_id_fake =$this->request->post['out_trade_no'];
            $order_id = end(explode('alipay', $order_id_fake));
			...
			$this->log->write('开始'.$order_info['order_status_id']);
			...
			if ($order_info) {
				...
				$this->log->write('中间'.$order_info['order_status_id']);
				...
				// if (!$order_info['order_status_id']) {
					...
					//注意：
						...
						$this->load->model('account/order');
						$shipping_code = $this->model_account_order->getOrderShippingcode($order_id);

						// 如果订单状态还是没有生成，说明是普通即时到账
						$this->log->write('为什么不打log啊');
						$this->log->write($order_info['order_status_id']);
						...
						if (!$order_info['order_status_id']) {
							...
							$this->model_checkout_order->addOrderHistory($order_id, $order_status_id,'普通即时到账，买家付款成功，订单状态自动修改');
						}else{
							$order_status_id = $this->config->get('alipay_direct_trade_finished_status_id');

							// 三个月后不需要改订单状态，只加了下comment
							$this->model_checkout_order->addOrderHistory($order_id, $order_info['order_status_id'],'高级即时到账，3个月已到');
						}
					...
					//注意：
						...
						if (!$order_info['order_status_id']) {
							...
							$this->model_checkout_order->addOrderHistory($order_id, $order_status_id,'高级即时到账，买家付款成功，订单状态自动修改');
