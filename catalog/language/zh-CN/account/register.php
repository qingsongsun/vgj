<?php

// Heading
$_['heading_title']        = '用户注册';

// Text
$_['text_account']         = '我的账户';
$_['text_register']        = '用户注册';
$_['text_account_already'] = '欢迎来到我们网站，如果您是新用户，请填写下面的表单进行注册。如果您已经是本站的会员，请您直接<a href="%s">登录</a>。';
$_['text_your_details']    = '您的个人资料';
$_['text_your_address']    = '您的详细地址';
$_['text_newsletter']      = '选择订阅方式';
$_['text_your_password']   = '您的账户密码（VIP卡号注册请输入卡密）';
$_['text_agree']           = '我已经阅读并同意 <a href="%s" class="agree"><b>%s</b></a>';

// Entry
$_['entry_customer_group'] = '客户类型：';
$_['entry_firstname']      = '您的名字：';
$_['entry_lastname']       = '您的姓氏：';
$_['entry_email']          = '电子邮箱：';
$_['entry_telephone']      = '联系电话：';
$_['entry_fax']            = '&nbsp;&nbsp;传真号码：';
$_['entry_company']        = '公司名称';
$_['entry_address_1']      = '地址 1：';
$_['entry_address_2']      = '&nbsp;&nbsp;地址 2：';
$_['entry_postcode']       = '&nbsp;&nbsp;邮政编码：';
$_['entry_city']           = '所在城市：';
$_['entry_country']        = '所在国家：';
$_['entry_zone']           = '地区/省份：';
$_['entry_newsletter']     = '订阅信息：';
$_['entry_password']       = '输入密码：';
$_['entry_confirm']        = '确认密码：';

// Error
$_['error_exists']         = '警告： 该邮箱地址已被注册、请换一个电子邮箱！';
$_['error_firstname']      = '名字必须在1到32字符之间！';
$_['error_lastname']       = '姓氏必须在1到32字符之间！';
$_['error_email']          = '无效的邮箱地址！';
$_['error_telephone']      = '电话号码必须为3到32字节！';
$_['error_address_1']      = '地址 1长度必须在3到128字符之间！';
$_['error_city']           = '城市名必须在2到128字节之间！';
$_['error_postcode']       = '邮编必须在2到10字符之间！';
$_['error_country']        = '请选择一个国家！';
$_['error_zone']           = '请选择地区/省份！';
$_['error_custom_field']   = '%s 必须填写！';
$_['error_password']       = '密码必须是 4 至 20 字符之间！';
$_['error_confirm']        = '两次输入的密码不同！';
$_['error_agree']          = '警告：您未有同意条款 %s！';

// Wenliang added below
$_['text_verification'] = '验证';
$_['entry_phone_or_card'] = '请输入手机号/VIP卡号：'; 
$_['text_sms_verification'] = '您的验证码是：%s。如需帮助请联系客服。';  // don't change this string since it will be verified again in ISP's template
$_['error_phone_or_card'] = '无效的号码';
$_['error_phone_exists'] = '该手机号已注册';
$_['error_card_exists'] = '该卡号已激活，请直接登录';
$_['error_card_password'] = '卡密码不正确';
$_['entry_vercode'] = "请输入短信验证码";
$_['entry_authcode'] = "请输入校验码"; 
$_['entry_sendvercode'] = "获取短信验证码"; 
$_['error_authcode'] = "错误的校验码"; 
$_['error_phone'] = "无效的手机号码";