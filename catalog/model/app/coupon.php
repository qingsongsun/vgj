<?php
class ModelAppCoupon extends Model {
	...
	public function getCouponByCodeCustomerid($data) {

		// $query=$this->db->query("SELECT * FROM ". DB_PREFIX ."coupon c LEFT JOIN ". DB_PREFIX ."customer_coupon cc on c.coupon_id=cc.coupon_id  WHERE code= '".$this->db->escape($data['coupon_code']) ."' AND customer_id='".(int)$data['customer_id']."'");

		$status = true;

		$coupon_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon c LEFT JOIN ".DB_PREFIX."customer_coupon cc on c.coupon_id=cc.coupon_id WHERE code = '" . $this->db->escape($data['coupon_code']) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1' AND customer_id='".(int)$data['customer_id']."'");
		// var_dump("SELECT * FROM " . DB_PREFIX . "coupon c LEFT JOIN ".DB_PREFIX."customer_coupon cc on c.coupon_id=cc.coupon_id WHERE code = '" . $this->db->escape($data['coupon_code']) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1' AND customer_id='".(int)$data['customer_id']."'");exit;
		...
		if ($coupon_query->num_rows) {
			...
			if ($this->customer->getId()) {
				$coupon_history_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch WHERE ch.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "' AND ch.customer_id = '" . (int)$data['customer_id']. "'");
	...
	public function getCouponForCustomer($coupon_id) {
	...
	public function dupilcateCoupon($data) {
		$this->event->trigger('pre.admin.coupon.add', $data);
		// $this->log->write('开始插入优惠券了');
		// $this->log->write($data);
		$this->db->query("INSERT INTO " . DB_PREFIX . "coupon SET name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "', discount = '" . (float)$data['discount'] . "', type = '" . $this->db->escape($data['type']) . "', total = '" . (float)$data['total'] . "', logged = '" . (int)$data['logged'] . "', shipping = '" . (int)$data['shipping'] . "', date_start = '" . $this->db->escape($data['date_start']) . "', date_end = '" . $this->db->escape($data['date_end']) . "', uses_total = '" . (int)$data['uses_total'] . "', uses_customer = '" . (int)$data['uses_customer'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
