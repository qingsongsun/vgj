...
class ModelCatalogReview extends Model {
	...
	// Wenliang added, for getting review content by customer id
	public function hasReviewByCustomerId($customer_id, $product_id) {
		// $query = $this->db->query("SELECT * FROM ".DB_PREFIX."review WHERE customer_id = '".(int) $customer_id."'");

		// huwen modified,for perfecting the logic
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."review WHERE customer_id = '".(int) $customer_id."'"."AND product_id='".(int) $product_id."'");

		// select * from oc_review where customer_id='$customer_id' and product_id='$prodcut_id';
		if ($query->num_rows == 0) {
			return false;
		} else {
		    return true;	
