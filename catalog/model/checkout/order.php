...
class ModelCheckoutOrder extends Model {
	...
	//ishanks added it
	public function addOrderHistoryShipping($order_id, $order_status_id, $comment = '', $notify = false, $shipping_data) {
		...
		if ($order_info) {
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW(), shipping_id = '".$shipping_data['shipping']."', shipping_name = '".$shipping_data['shipping_company']."', shipping_name_value = '".$shipping_data['shipping_company_pingyin']."'  WHERE order_id = '" . (int)$order_id . "'");

			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" .$shipping_data['shipping_company'].'('.$shipping_data['shipping_company_pingyin'].') '. $shipping_data['shipping']. '\n' . $this->db->escape($comment) . "', date_added = NOW()");
