...
Journal.enableSelectOptionAsButtonsList = function () {
        ...
        if ($option.hasClass('option-checkbox') || $option.hasClass('option-radio')) {
            $option.find('input').each(function () {
                ...
                if ($this.val()) {
                    ...
                    //add id to it so that we can find it and set it invisiable
                    html += '<li data-value="' + $this.val() + '" id="'+ $this.attr('id')+'_' +'" name="a"><span>' + $label.text().trim() + '</span></li>';
                }
            });
    ...
    $('.option-select li, .option-checkbox li, .option-radio li, .option-image li').click(function () {
        ...
        if ($option.hasClass('option-select')) {
            ...
            $option.find('input').each(function() {
                ...
                var val = $(this).val();
                if ($el.is(':checked')) {
                    //if it is mult-option and the option is aready selected, then unselect it
                    if($option.find('[data-value="' + val + '"]').hasClass('selected')&&$option.hasClass('multi-option')){
	...
	//ishanks added it to show the picture of the option when it is selected
	if($("#img"+val).size()>0) $("#img"+val).click();
...
if (Journal.isOC2) {
	...
	cart.add_bundle = function(product_list, quantity) {

		$.ajax({
			url: 'index.php?route=checkout/cart/add_bundle',
			type: 'post',
			data: 'product_list=' + product_list + '&quantity=' + (typeof(quantity) != 'undefined' ? quantity : 1),
			dataType: 'json',
			beforeSend: function() {
				$('#cart_bundle > a > span').button('loading');
			},
			complete: function() {
				$('#cart_bundle > a > span').button('reset');
			},
			success: function(json) {
               if (json['redirect']) {
                    location = json['redirect'];
...
function addBundleToCart(product_list, quantity) {

    quantity = typeof(quantity) != 'undefined' ? quantity : 1;

    if (Journal.isOC2) {
        return cart.add_bundle(product_list, quantity);
    }
...
//Huwen modified the addToWishlist function
if (Journal.isOC2) {
		...
		$.ajax({
			...
			success: function(json) {
				if (json['success']) {
                    ...
                    var list = document.getElementsByClassName('hint--top');
                    for(var i=0;i<list.length;i++){
                    var test = String(list[i].onclick);
                    test = test.split('addToWishList(\'');
                    if(test.length>1){
                        if(test[1].split('\'')[0]==String(product_id)){
                            var heart = document.getElementsByClassName('hint--top')[i].getElementsByTagName("i");
                            removeClass(heart[0],"wishlist-icon");
                            addClass(heart[0],"wishlist-icon-focused");
                }
            }
        }
				}
				...
				if (json['info']) {
                    ...
                    if(test.length>1){
                        if(test[1].split('\'')[0]==String(product_id)){
                            var heart = document.getElementsByClassName('hint--top')[i].getElementsByTagName("i");
                            removeClass(heart[0],"wishlist-icon-focused");
                            addClass(heart[0],"wishlist-icon");
                }
            }
        }
				}
